...
cold_war_nuclear_issues2 <- cold_war %>%
  mutate(month_decimal = as.numeric(month) * 0.08) %>%
  mutate(x_axis = as.numeric(year) + month_decimal) %>%
  group_by(x_axis) %>%
  mutate(count_issues = length(unique(rcid))) %>%
  select(x_axis, count_issues)
```

```{r, message=FALSE, warning=FALSE, include=TRUE}
font_add(family = "Pangolin", "Pangolin-Regular.ttf")
showtext.auto()
loadfonts()
```
...
plot <- ggplot(cold_war_nuclear_issues, aes(x=x_axis, y=count_issues, ymax=count_issues)) +
  geom_point(size=2, color='#D25c07') + 
  ...
  theme(
    ...
    plot.title = element_text(face = "bold", hjust = 0.5, color = "#581e03", size = 25), 
    plot.subtitle = element_text(hjust = 0.5, color = "#581e03", size = 18), 
    plot.caption = element_text(color = "#3f1602", size = 12)) + 
    dark_theme_gray(base_family="Pangolin")
plot
#ggsave("nuclear_plot.png")
```
